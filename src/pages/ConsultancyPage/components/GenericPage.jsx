...
const GenericPage = ({ data, isFlexReverse, isBackground, isTag, isChildHeading }) => {
    ...
    return (
        <>
            <div className={`${!isBackground ? "" : "bg-ai-consultancy-bg h-[1300px] bg-no-repeat bg-cover"}`}>
                                    ...
                                    <div className="flex w-fit items-start gap-4">
                                        ...
                                        {isChildHeading ? (
                                            <div className='flex flex-col'>
                                                <h2 className='text-black font-semibold text-[22px] cursor-pointer' onClick={() => handleToggle(index)} >
                                                    {service.heading}
                                                </h2>
                                                <AnimatePresence initial={false}>
                                                    {expandedIndex === index && (
                                                        ...
                                                        >
                                                            <p className={`${paragraph} w-[80%] ${isChildHeading ? "mt-2" : "ml-8"}`}>
                                                                {service.description}
                                                            </p>
                                                        </motion.div>
                                                    )}
                                            ...
                                            <p className={`${paragraph} w-[75%] ${isChildHeading ? "ml-11 mt-2" : "ml-3"}`}>
                                                {service.description}
                                            </p>
                                        </>}
                                    </div>
